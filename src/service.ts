...
import parseNaturalLanguageQuery from "./utils/query-parser";
...
const service = {
  ...
  async searchProfiles(query: string): Promise<ServiceResponse<any>> {
    const filters = parseNaturalLanguageQuery(query.toLowerCase());

    if (Object.keys(filters).length === 0) {
      return {
        statusCode: 400,
        error: new Error('Unable to interpret query'),
      }
    }
    
    const matchingProfiles = await repository.queryProfilesWithNLP(filters)
    return {
      statusCode: 200,
